...
class Youtube
{
    ...
    public function search_MY_Advanced($params, $pageInfo = false)
    {
        $API_URL = $this->getApi('search.list');
    ...
    public function get_MY_channels(array $params = ['part' => 'snippet,contentDetails,statistics', 'mine' => 'true'])
    {
        $API_URL = $this->getApi('channels.list');
        $params = $params;
    ...
    public function get_MY_channel(array $params = ['part' => 'snippet,contentDetails,statistics', 'mine' => 'true'])
    {
        $API_URL = $this->getApi('channels.list');
        $params = $params;
    ...
    public function get_MY_Playlists($optionalParams = [], $part = ['id', 'snippet', 'status'])
    {
        $API_URL = $this->getApi('playlists.list');
        $params = [
            'mine' => 'true',
    ...
    public function create_MY_Playlist($title, $desc = '', $optionalParams = [], $part = ['id', 'snippet', 'status'])
    {
        $API_URL = $this->getApi('playlists.list');
        $params = [
            'mine' => 'true',
        ...
        $post = [];
        $post['snippet'] = [];
        $post['snippet']['title'] = $title;
        if ($desc) {
            $post['snippet']['description'] = $desc;
        }

        $apiData = $this->api_get($API_URL, $params, $post);


        $result = ['results' => $this->decodePostSingle($apiData)];
        return $result;
    }


    public function add_MY_VideoToPlaylist($playlistId, $videoId, $optionalParams = [], $part = ['id', 'snippet', 'status'])
    {
        ...
        $post = [];
        $post['snippet'] = [];
        $post['snippet']['playlistId'] = $playlistId;
        $post['snippet']['resourceId'] = [];
        $post['snippet']['resourceId']['kind'] = 'youtube#video';
        $post['snippet']['resourceId']['videoId'] = $videoId;
    ...
    public function delete_MY_Playlist($id)
    {
        $API_URL = $this->getApi('playlists.list');
        $params = [
            'id' => $id,
        ...
        $apiData = $this->api_get($API_URL, $params, 'delete');


        $result = ['results' => $this->decodeDeleteSingle($apiData)];
        return $result;

    }

    public function delete_MY_VideoFromPlaylistById($id)
    {
        $API_URL = $this->getApi('playlistItems.list');
        $params = [
            'id' => $id,
    ...
    public function get_MY_ChannelVideos(array $part = ['id', 'snippet'], $pageInfo = false)
    {
        $params = [
            'forMine' => 'true',
            'type' => 'video',
        ...
        return $this->search_MY_Advanced($params, $pageInfo);
    }

    public function get_MY_PlaylistById($id, $part = ['id', 'snippet', 'status'])
    {
        $API_URL = $this->getApi('playlists.list');
        $params = [
            'id' => is_array($id) ? implode(',', $id) : $id,
        ...
        return ['results' => $this->decodeSingle($apiData)];
    }

    public function get_MY_PlaylistItemsByPlaylistId($playlistId, $pageToken = '', $maxResults = 50, $part = ['id', 'snippet', 'contentDetails', 'status'])
    {
        $API_URL = $this->getApi('playlistItems.list');
        $params = [
    ...
    public function get_MY_VideoById($vId, $part = ['id', 'snippet', 'contentDetails', 'player', 'statistics', 'status'])
    {
        $API_URL = $this->getApi('videos.list');
        $params = [
            'id' => is_array($vId) ? implode(',', $vId) : $vId,
        ...
        if (is_array($vId)) {
            return ['results' => $this->decodeMultiple($apiData)];
        }
    ...
    public function decodeDeleteSingle(&$apiData)
    {

        $resObj = json_decode($apiData);

        if (isset($resObj->return) && $resObj->return == 'error') {
            return $resObj;
        }
    ...
    public function decodePostSingle(&$apiData)
    {
     ...
     * @throws \Exception
     */
    public function api_get($url, $params, $method = [])
    {
        $params['key'] = $this->youtube_key;

        //boilerplates for CURL

        $accessToken = app('request')->getSession()->get('user')->google;
        ...
        if ($accessToken === false) {
            ...
            return json_encode([
                'return' => 'error',
                'job' => 'auth',
                'message' => 'Access Token not found or not valid.'
            ]);
        ...
        if ($method == 'delete') {
            ...
            curl_setopt($tuCurl, CURLOPT_POSTFIELDS, json_encode($method));
        }
